...
export const resolver_v3 = (
  opType: 'init' | 'mint' | 'tele' | 'burn' | 'call' | 'send' | 'dele' | 'stak' | 'mine',
  keypair: { childNodeXOnlyPubkey: Buffer },
  cborPayload: CborPayload,
  totalFee: number,
  fundingUtxo: {
    txid: string;
  ...
  const nonceStart = nonce_pre_head / 2 + 5 + 1;
  const nonceEnd = nonceStart + 4;
  const op_return_hex = test_return_hex ?? '9d4b1212d0c917e6'; //'f000000000000000000000000000000f';
  const op_return_asm = `OP_RETURN ${op_return_hex}`;
  const op_return_script = bitcoin.script.fromASM(op_return_asm);
  let _network = network === 'testnet' ? bitcoin.networks.testnet : bitcoin.networks.bitcoin;
  const head = cbor.slice(0, nonceStart); // nonce is going to insert after head
  const { hashscript, scriptP2TR } = prepareDodScripts(keypair.childNodeXOnlyPubkey, opType, cborPayload.cbor(), _network);
  ...
  // console.log({ simulateTx: simulateTx.getId() });
  ...
  return {
    ...
    tx,
    internalPubkey: keypair.childNodeXOnlyPubkey.toString('hex'),
    network: _network === bitcoin.networks.testnet ? 'testnet' : 'mainnet',
  };
};
